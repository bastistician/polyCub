...
## Storing figures in man/figures would inflate the package.
## Instead we .Rbuildignore the generated figures and use the full GitHub path
## when referencing the figures in the resulting markdown file
knitr::opts_knit$set(
  base.url = "https://raw.githubusercontent.com/bastistician/polyCub/master/"
)
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "figures/", dev = "png", dpi = 96,
  fig.width = 4.5, fig.height = 3.75, dev.args = list(pointsize = 10)
)
```
```{R example-f, include = FALSE}
f <- function (s, sigma = 5)
{
    exp(-rowSums(s^2)/2/sigma^2) / (2*pi*sigma^2)
}
```
```{R example-polygon, include = FALSE}
hexagon <- list(
    list(x = c(7.33, 7.33, 3, -1.33, -1.33, 3),
         y = c(-0.5, 4.5, 7, 4.5, -0.5, -3))
)
```
```{R logo, fig.width = 1.3, fig.height = 1.5, include = FALSE}
...
text(3, 2, labels = "\u222B", cex = 5)  # Unicode integral symbol
```

# polyCub <img src="`r paste0(knitr::opts_knit$get("base.url"), "figures/logo-1.png")`" align="right" />
...
```{R product-Gauss, echo = -1, fig.cap = "Product Gauss cubature", fig.show = "hold"}
par(mar = c(3,3,1,2))
...
```{R midpoint, echo = -1, fig.cap = "Midpoint rule", fig.show = "hold"}
