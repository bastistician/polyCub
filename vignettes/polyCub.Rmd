...
## accommodate to _R_CHECK_DEPENDS_ONLY_VIGNETTES_=TRUE
HAVE_SPATSTAT <- requireNamespace("spatstat.geom", quietly = TRUE)
## conditionally accept gpclib license (prohibits commercial use)
if (USE_GPCLIB <- identical(Sys.getenv("R_GPCLIBPERMIT"), "true"))
    polyCub::gpclibPermit()
## skip benchmark in regular checks
DO_BENCHMARK <- HAVE_SPATSTAT && identical(Sys.getenv("NOT_CRAN"), "true")
```
...
```{R, message = FALSE, purl = FALSE, eval = HAVE_SPATSTAT}
...
```{R midpoint, echo = -1, fig.show = "hold", purl = FALSE, eval = HAVE_SPATSTAT}
...
```{R, purl = FALSE, eval = HAVE_SPATSTAT && USE_GPCLIB}
